//1.1)[schermata amici se non hai amici] suggest users according to categories ( va a guardare se c'è almeno una categoria in comune, ritorna lista utenti, divisione tra suggeriti (una in comune) e *best match* (con tutte e due in comune) ) 
...
//1)[schermata amici se non hai amici] suggest users according to categories ( va a guardare se c'è almeno una categoria in comune, ritorna lista utenti, divisione tra suggeriti (una in comune) e *best match* (con tutte e due in comune) ) 
OPTIONAL MATCH (ub:User),(ua:User)
WHERE ub.id="fd545635" AND NOT EXISTS ((ub)-[:FOLLOW]->(ua))
AND NOT EXISTS((ua)-[:FOLLOW]->(ub)) AND
